...
#include "CameraMode/CitySampleCam_ThirdPerson.h"
...
{
	const ACharacter* const Char = Cast<ACharacter>(ViewTarget);
...
{
	const APlayerController* const PC = PlayerCamera ? Cast<APlayerController>(PlayerCamera->PCOwner) : nullptr;
	if (PC == nullptr)
	{
