...
void UEventsK2Node_NotifyEvent::ExpandNode(class FKismetCompilerContext& CompilerContext, UEdGraph* SourceGraph)
{
	...
	UEdGraphPin* CallSenderPin = CallNotifyFuncNode->FindPinChecked(FPinName::SenderPinName);
	bError &= CompilerContext.MovePinLinksToIntermediate(*SpawnSenderPin, *CallSenderPin).CanSafeConnect();

	UEdGraphPin* CallListenerPin = CallNotifyFuncNode->FindPinChecked(FPinName::ListenerPinName);
...
{
	const UEdGraphSchema_K2* K2Schema = GetDefault<UEdGraphSchema_K2>();
	UEdGraphPin* Pin = CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Wildcard, FPinName::ListenerPinName);
	DefaultPins.Add(Pin);
