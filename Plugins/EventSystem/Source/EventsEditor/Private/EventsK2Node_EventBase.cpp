...
FName FPinName::DefaultPinName = FName(TEXT("Default"));
FName FPinName::EventPinName = FName(TEXT("CustomEvent"));
FName FPinName::SenderPinName = FName(TEXT("Sender"));
FName FPinName::ListenerPinName = FName(TEXT("Listener"));
...
void UEventsK2Node_EventBase::CreateSelectionPin()
{
	...
	UEdGraphPin* Pin = CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Struct, FEventInfo::StaticStruct(), FPinName::EventPinName);
	DefaultPins.Add(Pin);
...
{
	return FindPin(FPinName::EventPinName);
}

UEdGraphPin* UEventsK2Node_EventBase::GetListenerPin() const
{
	return FindPin(FPinName::ListenerPinName);
}
